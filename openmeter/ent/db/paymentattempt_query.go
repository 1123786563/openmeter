...
func (_q *PaymentAttemptQuery) loadFacts(ctx context.Context, query *PaymentFactQuery, nodes []*PaymentAttempt, init func(*PaymentAttempt), assign func(*PaymentAttempt, *PaymentFact)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(paymentfact.FieldPaymentAttemptID)
	}
	...
	for _, n := range neighbors {
		fk := n.PaymentAttemptID
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "payment_attempt_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
