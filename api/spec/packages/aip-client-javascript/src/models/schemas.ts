  ...
  .object({
    ...
    config: z
      .unknown()
        ...
        .describe("The entitlement config as a JSON object. Returned when checking entitlement access; useful for configuring fine-grained access settings implemented in your own system."),
  })
  ...
  .object({
    ...
    dueAfter: z
      ...
      .optional()
        ...
        .describe("The period after which the invoice is considered overdue if not paid."),
  })
...
export const aiUsageBillingMode = z
  .enum(["component", "bundle"])
    ...
    .describe("Determines how a usage batch is charged. - `component`: Per-resource pricing; each line item is rated individually. - `bundle`: Flat ceiling charge; no per-resource breakdown.");
  ...
  .object({
    ...
    quantity: z.coerce
      ...
      .lte(9223372036854775807n)
        ...
        .describe("Raw consumption quantity (tokens, calls, seconds, pages, or images)."),
    provider: z
      .string()
      .optional()

        .describe("LLM provider (e.g. `openai`, `anthropic`). Required for provider-managed resources."),
    model: z
      .string()
      .optional()

        .describe("Canonical model identifier (e.g. `gpt-4o`, `claude-3-5-sonnet`). Required for provider-managed resources."),
    pricingDimensions: z
      .record(z.string(), z.string())
      .optional()

        .describe("Optional resource-specific pricing dimensions (e.g. region, tier)."),
    canonicalLineIndex: z
      ...
      .lte(2147483647)
        ...
        .describe("Stable zero-based position used for deterministic ceiling allocation when the batch total exceeds the reservation ceiling."),
  })
...
export const aiUsageBatchStatus = z
  .enum(["settled", "corrected"])
    ...
    .describe("Settlement status of a usage batch. - `settled`: Batch processed and credits deducted successfully. - `corrected`: Batch reprocessed via a correction or compensation entry.");
  ...
  .object({
    subjectKey: z
      ...
      .optional()
        ...
        .describe("The subject or tenant key that will produce the usage. Required to check the tenant's sequence watermark and reservation status."),
    reservationId: z
      .string()
      .optional()
...
export const commerceFenAmount = z.coerce
  ...
  .lte(9223372036854775807n)
    ...
    .describe("Integer amount in the smallest currency unit (e.g. CNY fen, USD cents). Used for all payment, order, refund and invoice monetary fields.");

export const commerceOrderKind = z
  .enum(["plan_purchase", "subscription_renewal", "wallet_top_up"])

    .describe("The business type of an order. - `plan_purchase`: Initial subscription purchase. - `subscription_renewal`: Renewal of an existing subscription. - `wallet_top_up`: Prepaid credit recharge.");
...
export const commercePaymentProvider = z
  .enum(["wechat", "alipay", "offline"])
    ...
    .describe("The payment channel that processed (or will process) a payment. - `wechat`: WeChat Pay. - `alipay`: Alipay. - `offline`: Offline bank transfer or enterprise remittance.");
...
export const aiUsageErrorCode = z
  .enum([
    ...
    .describe("Machine-readable error codes specific to AI Usage billing. These codes appear in the `error_code` field of RFC-7807 problem responses returned by AI Usage endpoints.");
...
export const commercePlanRef = z
  .object({
    ...
    .describe("A lightweight reference to a plan in the product catalog, embedded in order and catalog responses without duplicating the full plan definition.");
...
export const commerceSubscriptionRef = z
  .object({
    ...
    .describe("A lightweight reference to a subscription, embedded in order responses.");
  ...
  .object({
    ...
    reservationCeilingCredits: z.coerce
      ...
      .optional()
        ...
        .describe("The reservation ceiling in Credits, if a reservation is active. When set, usage is capped at this amount per reservation period."),
    coveredTenantSeq: z.coerce
      .bigint()
      .gte(-9223372036854775808n)
      .lte(9223372036854775807n)

        .describe("The highest continuously covered tenant sequence number, used for watermark tracking."),
    denialReason: z
      .string()
      .optional()

        .describe("The reason authorization was denied, when `authorized` is `false`."),
    canonicalPayload: z
      .record(z.string(), z.unknown())
      .optional()

        .describe("Canonical JSON payload for consumer-side verification. The raw bytes of the canonicalized authorization object, used to derive the SHA-256 hash and verify the signature."),
    canonicalSha256: z
      .string()
      .optional()
    ...
    .describe("Runtime authorization decision for a customer's AI usage. Returned before processing a usage batch to indicate whether the customer is authorized to consume AI resources, along with their current credit balance and reservation ceiling.");
...
export const aiUsageCreditBalance = z
  .object({
    ...
    .describe("Integer credit balance for AI usage, scoped to a single currency. Unlike the OpenMeter Credits decimal balance, AI Usage tracks credits as signed 64-bit integers to avoid floating-point rounding in settlement.");
...
export const commerceExternalInvoiceUpdate = z
  .object({
    ...
    .describe("Request body for attaching or updating an external invoice reference on a receivable period (e.g. a VAT invoice number from the external tax service).");
  ...
  .object({
    ...
    limit: z
      ...
      .optional()
        ...
        .describe("The amount of usage granted each usage period, in the feature's unit. Usage is counted against this allowance and the balance resets every usage period. When `is_soft_limit` is true the subject keeps access after the limit is reached; otherwise access is denied once the allowance is exhausted."),
  ...
  .object({
    idempotencyKey: z
      .string()
        ...
        .describe("Client-generated idempotency key. Replaying the same key with the same `payload_hash` returns the stored result with HTTP 200. A replay with a different `payload_hash` for the same key returns HTTP 409."),
    payloadHash: z
      .string()

        .describe("SHA-256 hex digest of the canonical request body, used for idempotency verification."),
    ...
    tenantSeq: z.coerce
      ...
      .lte(9223372036854775807n)
        ...
        .describe("Monotonic per-tenant sequence number for watermark tracking. Must be strictly increasing within a tenant."),
    occurredAt: dateTime,
    reservationId: z
      .string()
    ...
    reservationCeilingCredits: z.coerce
      ...
      .lte(9223372036854775807n)
        ...
        .describe("Caps the total Credit charge for this batch. The platform absorbs any excess above the ceiling."),
    ...
    providerManaged: z
      .boolean()
        ...
        .describe("Whether model resources are platform-managed. Set to `false` for bring-your-own-key (BYOK) models."),
    lines: z
      .array(aiUsageUsageLineCreate)
      .min(1)

        .describe("The individual resource consumption entries. Must contain at least one line for `component` billing mode."),
  })

    .describe("Request body for submitting a Canonical AI Usage Batch. A batch represents one business action (a chat turn, an agent run, etc.) and is the atomic unit of AI billing settlement.");
  ...
  .object({
    ...
    amount: z.coerce
      ...
      .lte(9223372036854775807n)
        ...
        .describe("Signed credit amount. Positive adds balance, negative reduces it."),
    ...
    .describe("An immutable credit movement on the AI Usage integer credit ledger.");
  ...
  .object({
    ...
    refundableCredits: z.coerce
      ...
      .optional()
        ...
        .describe("Credits that are refundable (only present for recharge-source buckets). Represents the unspent portion of purchased credits."),
  })

    .describe("A single credit bucket in a customer's Wallet. Each bucket represents credits from one funding source with its own balance and expiration rules. The Wallet is a read-only aggregation over the immutable Credit Ledger; it never holds a mutable second balance.");
  ...
  .object({
    ...
    priority: z
      ...
      .lte(2147483647)
        ...
        .describe("Settlement priority (0 = plan credit, 10 = gift, 20 = recharge, 30 = enterprise receivable)."),
    source: commerceWalletBucketSource,
  })

    .describe("Immutable ledger provenance attached to a Wallet transaction. Records the underlying ledger entry that backs the credit movement, without exposing internal Ent IDs.");
...
export const commerceRechargeProduct = z
  .object({
    ...
    .describe("A recharge product (SKU) available for purchase. Each product defines a credit package and its retail price in integer fen.");
...
export const commerceRefundCreate = z
  .object({
    ...
    .describe("Request body for creating a refund. Refunds are only permitted for unspent recharge-source credits. The system enforces a billing fence before reversing any ledger entries.");
  ...
  .object({
    ...
    externalReference: z
      .string()
        ...
        .describe("External reference (bank transfer number, remittance advice)."),
    receivedAt: dateTime,
    note: z.string().optional().describe("Optional note from the submitter."),
  })

    .describe("Request body for recording an offline payment (bank transfer, enterprise remittance) against a customer's account.");
...
export const commerceRechargeProductWithBonus = z
  .object({
    ...
    .describe("A recharge product with promotional bonus credits (e.g. \"buy 1000 get 100 extra\"). Bonus credits are granted as a separate gift-source bucket.");
  ...
  .object({
    ...
    externalReference: z
      ...
      .optional()
        ...
        .describe("For offline payments: the external reference (e.g. bank transfer number)."),
  })
  ...
  .object({
    ...
    paymentUrl: z
      ...
      .optional()
        ...
        .describe("Provider-specific payment URL or QR code payload (e.g. WeChat code_url)."),
    providerOrderId: z
      .string()
      .optional()
    ...
    .describe("A checkout session for an order, representing a single payment attempt.");
...
export const commercePaymentFact = z
  .object({
    ...
    .describe("A verified payment fact from a payment provider. Represents a confirmed monetary transfer without exposing raw credentials.");
...
export const commerceRefund = z
  .object({
    ...
    .describe("A refund request, progressing through the billing fence and provider processing before the ledger is reversed.");
...
export const commerceReceivablePeriod = z
  .object({
    ...
    .describe("An enterprise receivable period representing one monthly settlement cycle. Enterprise customers with approved contracts can consume on credit and settle via offline payments against these periods.");
...
export const aiUsageSalesSnapshot = z
  .object({
    ...
    .describe("The customer-facing sales price snapshot (typically in CNY for display).");
  ...
  .object({
    ...
    credits: z.coerce
      ...
      .optional()
        ...
        .describe("Credits that will be granted on fulfillment (for top-up and plan orders)."),
    status: commerceOrderStatus,
    businessTrackingNumber: z
      .string()
      .optional()

        .describe("Human-readable business tracking number shown to the customer."),
    createdAt: dateTime,
    updatedAt: dateTime,
    expiredAt: dateTime.optional(),
  })

    .describe("A commerce order representing a purchase intent (plan, renewal, or top-up).");
...
export const appStripeCreateCheckoutSessionConsentCollection = z
  .object({
    ...
    promotions: appStripeCreateCheckoutSessionConsentCollectionPromotions
      .optional(),
    termsOfService: appStripeCreateCheckoutSessionConsentCollectionTermsOfService
      .optional(),
  })
...
export const rateCardEntitlement = z
  .discriminatedUnion(
    ...
    .describe("Entitlement template configured on a rate card. The feature is taken from the rate card itself, so it is omitted here.");
...
export const commerceWalletTransaction = z
  .object({
    ...
    .describe("An immutable credit movement on the Wallet, projected from the Credit Ledger. Every transaction carries immutable ledger provenance and a timestamp.");
...
export const aiUsageUsageLine = z
  .object({
    ...
    .describe("A rated usage line returned in a settled batch, with resolved cost and sales snapshots.");
...
export const aiUsageRatingSnapshot = z
  .object({
    ...
    .describe("A rating snapshot captures the cost and sales price resolution for one line item within a settled batch.");
...
export const appStripeCreateCheckoutSessionRequestOptions = z
  .object({
    billingAddressCollection: appStripeCreateCheckoutSessionBillingAddressCollection
      .optional()
      .default("auto"),
    cancelUrl: z
      .string()
    ...
    redirectOnCompletion: appStripeCreateCheckoutSessionRedirectOnCompletion
      .optional(),
...
export const commerceWallet = z
  .object({
    ...
    .describe("The aggregate Wallet view for a customer, combining all credit buckets and recent transactions.");
  ...
  .object({
    ...
    totalCredits: z.coerce
      ...
      .lte(9223372036854775807n)
        ...
        .describe("Total integer Credit charge for the batch, after applying the reservation ceiling."),
    coveredTenantSeq: z.coerce
      .bigint()
      .gte(-9223372036854775808n)
      .lte(9223372036854775807n)

        .describe("The highest continuously covered tenant sequence number after settlement."),
    createdAt: dateTime,
  })
...
export const aiUsageBatchSettlementResult = z
  .object({
    ...
    .describe("Settlement result for a batch, including the ledger entries burned and the watermark advancement.");
  ...
  .object({
    ...
    affectedSubscriptionItemIds: z
      .array(ulid)
        ...
        .describe("The IDs of the subscription items that this rate card belongs to."),
  })
...
export const getCustomerRuntimeAuthorizationResponse = aiUsageRuntimeAuthorization;
...
export const appStripeCreateCheckoutSessionConsentCollectionPaymentMethodReuseAgreementPositionWire = z
  .enum(["auto", "hidden"])
  .describe("Position of payment method reuse agreement in the UI.");
...
export const appStripeCreateCheckoutSessionConsentCollectionTermsOfServiceWire = z
  .enum(["none", "required"])
  .describe("Terms of service acceptance requirement.");
...
export const unsupportedMediaTypeWire = baseErrorWire
  .describe("Unsupported Media Type.");

export const unprocessableContentWire = baseErrorWire
  .describe("Unprocessable Content.");
...
export const appStripeCreateCheckoutSessionConsentCollectionPaymentMethodReuseAgreementWire = z
  .strictObject({
    position: appStripeCreateCheckoutSessionConsentCollectionPaymentMethodReuseAgreementPositionWire
      .optional(),
  })
  .describe("Payment method reuse agreement configuration.");
...
export const appStripeCreateCheckoutSessionConsentCollectionWire = z
  .strictObject({
    payment_method_reuse_agreement: appStripeCreateCheckoutSessionConsentCollectionPaymentMethodReuseAgreementWire
      .optional(),
    promotions: appStripeCreateCheckoutSessionConsentCollectionPromotionsWire
      .optional(),
    terms_of_service: appStripeCreateCheckoutSessionConsentCollectionTermsOfServiceWire
      .optional(),
  })
  .describe("Checkout Session consent collection configuration.");
...
export const featureUnitCostWire = z
  .discriminatedUnion(
    "type",
    [featureManualUnitCostWire, featureLlmUnitCostWire]
  )
  ...
  .strictObject({
    ...
    client_reference_id: z
      ...
      .optional()
        ...
        .describe("Unique reference string for reconciling sessions with internal systems. Can be a customer ID, cart ID, or any other identifier."),
    customer_update: appStripeCreateCheckoutSessionCustomerUpdateWire
      .optional(),
    consent_collection: appStripeCreateCheckoutSessionConsentCollectionWire
      .optional(),
    currency: currencyCodeWire.optional(),
    ...
    payment_method_types: z
      ...
      .optional()
        ...
        .describe("List of payment method types to enable (e.g., \"card\", \"us_bank_account\"). If not specified, Stripe enables all relevant payment methods."),
    redirect_on_completion: appStripeCreateCheckoutSessionRedirectOnCompletionWire
      .optional(),
    tax_id_collection: appStripeCreateCheckoutSessionTaxIdCollectionWire
      .optional(),
  })
...
export const priceUsageBasedWire = z
  .discriminatedUnion(
    "type",
    [priceUnitWire, priceGraduatedWire, priceVolumeWire]
  )
...
export const appWire = z
  .discriminatedUnion(
    "type",
    [appStripeWire, appSandboxWire, appExternalInvoicingWire]
  )
  .describe("Installed application.");
...
export const createChargeRequestWire = z
  .discriminatedUnion(
    "type",
    [createChargeFlatFeeRequestWire, createChargeUsageBasedRequestWire]
  )
  .describe("Customer charge.");
...
export const updateCustomerBillingBodyWire = upsertCustomerBillingDataRequestWire;
...
export const updateCustomerBillingAppDataBodyWire = upsertAppCustomerDataRequestWire;
...
export const createCustomerStripeCheckoutSessionBodyWire = customerStripeCreateCheckoutSessionRequestWire;

export const createCustomerStripeCheckoutSessionResponseWire = appStripeCreateCheckoutSessionResultWire;

export const createCustomerStripePortalSessionPathParamsWire = z.object({
  customerId: ulidWire,
});

export const createCustomerStripePortalSessionBodyWire = customerStripeCreateCustomerPortalSessionRequestWire;

export const createCustomerStripePortalSessionResponseWire = appStripeCreateCustomerPortalSessionResultWire;
...
export const updateCreditGrantExternalSettlementBodyWire = updateCreditGrantExternalSettlementRequestWire;
...
export const createSubscriptionAddonBodyWire = createSubscriptionAddonRequestWire;
...
export const getCustomerRuntimeAuthorizationResponseWire = aiUsageRuntimeAuthorizationWire;
