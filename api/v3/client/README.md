...
| `om.Commerce.AlipayPaymentCallback` | `POST /payment-providers/alipay/callback` | Alipay payment callback. OpenMeter verifies the signature, confirms the payment fact, and fulfills the order. The request body is the `application/x-www-form-urlencoded` Alipay notification whose field set varies by trade type. OpenMeter verifies the RSA2 signature over the canonicalized raw form fields, so the body is intentionally not modeled with a schema and must be forwarded byte-for-byte. |
